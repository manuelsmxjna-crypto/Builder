...
const sheetWidth=()=>Math.max(10,Number(BRIDGE.sheetWidth?.()||BRIDGE.FIXED_SHEET_WIDTH_CM||62));
...
function applyAllSheetsResult(result,problem,sourceObjects){
  ...
  const newSheets=Array.from({length:result.sheetCount},(_,i)=>({
    id:`gs${i+1}`,
    name:`Gang Sheet ${i+1}`,
    sheet:{w:sheetWidth(),h:MIN_SHEET_HEIGHT,margin:0},
    objects:[]
  }));
...
async function quoteRectangles(rows,{gapCm=0,allowRotate=true}={}){
  const activeWidth=sheetWidth();
  const objects=[];
  for(const row of rows||[]){
    const qty=Math.max(1,Math.round(Number(row.qty)||1));
    const w=Math.max(.1,Number(row.w)||.1);
    const h=Math.max(.1,Number(row.h)||.1);
    if(Math.min(w,h)>activeWidth || Math.max(w,h)>MAX_SHEET_HEIGHT){
      throw new Error(`${row.name||"Medida"}: ${w.toFixed(1)} × ${h.toFixed(1)} cm no cabe en una hoja de ${activeWidth} cm.`);
    }
